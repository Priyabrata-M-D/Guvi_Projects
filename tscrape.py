...
def scrape_twitter_data(hashtag, start_date, end_date, max_tweets):
    ...
    for i, tweet in enumerate(
        ...
        if i >= max_tweets:
            break
        tweets_list.append(
            [tweet.date, tweet.id, tweet.url, tweet.user.username, tweet.replyCount, tweet.content, tweet.retweetCount,
             tweet.lang, tweet.source, tweet.likeCount])
    tweets_df = pd.DataFrame(tweets_list,
                             columns=["Date", "ID", "URL", "User", "Reply Count", "Content", "Retweet Count",
                                      "Language",
                                      "Source", "Like Count"])
    return tweets_df
